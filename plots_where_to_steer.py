...
methods = ['atp', 'atp-zero', 'acp', 'probes']
topks = [0.05]
    ...
    for method in methods:
        ...
        for topk in topks:
            for sf in steering_factors:
                try:
                    file_path  = f"./runs/{model_id}/from_{source_task}_to_{base_task}/None_eval/{method}-heads/{sf}/no_base_logits/eval-test/global/{metric}/targeted_steer_{metric}_{topk}_gen_accuracy.json"
                    with open(file_path, "r") as f:
                        acc = json.load(f)
                        q1 = acc['q1']
                except FileNotFoundError:
                    print(f"File path {file_path} not found. Using random value.")
...
def load_control_data():
    ...
    for method in methods:
        ...
        for topk in topks:
            for sf in steering_factors:
                try:
                    file_path  = f"./runs/{model_id}/from_{source_task}_to_{base_task}/None_eval/atp-heads/{sf}/no_base_logits/eval-test/global/{metric}/random_steer_{metric}_{topk}_gen_accuracy.json"
                    with open(file_path, "r") as f:
                        acc = json.load(f)
                        q1 = acc['q1']
                except FileNotFoundError:
                    print(f"File not found for method {method}, topk {topk}, sf {sf}. Using random value.")
                    # q1 = np.random.rand()
                data[(method, topk, sf)] = q1
    return data
            ...
            for topk in topks:
                ...
                for method in methods:
                    ...
                    if acc is not None and control is not None:
                        acc_row[method.upper()] = f"{acc:.2f}"
                        acc_row["RANDOM"] = f"{control:.2f}"
                        if acc >= ACC:
                            found_any = True
