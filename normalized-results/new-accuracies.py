...
triplet_ids = [re.search(r"jp_(\d+)\.", f).group(1) for f in jp_files]

print("Detected triplets:", triplet_ids)
...
if not (
    ...
    for tid in triplet_ids:
        ...
        jp_path     = f"jp_{tid}.judge_prompt.judge_outputs.json"
        rf_flu_path = f"rf_{tid}.fluency_prompt.judge_outputs.json"
        rf_rel_path = f"rf_{tid}.relevance_prompt.judge_outputs.json"
    ...
    jp_df     = [jp_df[i][j][k]     for i in range(len(jp_df))     for j in range(len(jp_df[i]))     for k in range(len(jp_df[i][j]))]
    rf_flu_df = [rf_flu_df[i][j][k] for i in range(len(rf_flu_df)) for j in range(len(rf_flu_df[i])) for k in range(len(rf_flu_df[i][j]))]
    rf_rel_df = [rf_rel_df[i][j][k] for i in range(len(rf_rel_df)) for j in range(len(rf_rel_df[i])) for k in range(len(rf_rel_df[i][j]))]
