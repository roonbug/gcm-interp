...
models = ["SOLAR-10.7B-Instruct-v1.0-steering-all"]
...
save_dir = f"{RM_INTERP_REPO}/results/accuracy-single/plots/"
...
for model_id in models:
    for ablation in ["steer"]:
        if ablation in ["steer", "mean"]:
            root_dir = f"{RM_INTERP_REPO}/results/accuracy-single/{model_id}"
        else:
            root_dir = f"{RM_INTERP_REPO}/results/accuracy-single/{model_id}"
        ...
        colormaps = [cm.get_cmap('Greens')] * 4
            ...
            for col_idx, method in enumerate(methods):
                ...
                for i, sf in enumerate(steering_factors):
                    for j, topk in enumerate(topk_values):
                        ...
                        if topk == 1:
                            load_method = "atp"     # ALWAYS read from acp dir when topk==1
                        else:
        ...
        # Save heatmap grid
        plt.suptitle(f"{model_id}", fontsize=28)
